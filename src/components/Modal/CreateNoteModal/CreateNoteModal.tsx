...
const CreateNoteModal = () => {
  ...
  const { veiwAddTagsModal } = useAppSelector((state) => state.modal);
  ...
  const tagsHandler = (tagName: string, type: string) => {
    const newTagName = tagName.toLocaleLowerCase();

    if (type === "add") {
      setAddedTags((prev) => [...prev, { tagName: newTagName, id: v4() }]);
    } else {
      setAddedTags(addedTags.filter(({ tagName }) => tagName !== newTagName));
    }
  };
  ...
  return (
    <FixedContainer>
      ...
      <Box>
        ...
        <AddedTagsBox>
          {addedTags.map(({ tagName, id }) => (
            <div key={id}>
              <span className="createNote__tag">{tagName}</span>
              <span
                className="createNote__tag-remove"
                onClick={() => tagsHandler(tagName, "remove")}
              >
                <FaTimes />
              </span>
            </div>
